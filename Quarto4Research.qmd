...
::: {.callout-tip title="Don't let typos ruin your Quarto flow!"}
...
[**Q6**]{style="color:green;"} What is the output of the "Package installation and loading" chunk in your rendered `.html` document?

```{r}
#| echo: false
#| label: "Q6"

check_question("Three messages, one per loaded library.",
               options = c("Nothing.", 
                           "Three messages, one per loaded library.", 
               ...
               q_id = "Q6", 
button_label = "Check answer",
right = "That's right! What you are seeing are messages that the {here}, {tidyverse}, and {xfun} libraries automatically output when they are correctly loaded.",
wrong = "Oh no, something's gone wrong... If you are getting the fourth error message, this means that your document could not be rendered because one of the three packages has not been installed yet. Install them and then try rendering your document again. Also, check that you have not misspelt the names of the packages! If you are getting the last error message, there is a problem with your Quarto document, which means that it cannot be rendered. Read the rest of the error message to understand where the problem lies. It is very likely to be a small syntax error or typo.") 
check_hint("Remember that a conflict message is *not* an error message, it merely informs us about potential conflicts when two different packages have functions with the same name.", 
           hint_title = "<br>🐭 Click on the mouse for a hint.")
```

[**Q7**]{style="color:green;"} Which code chunk option can you use to remove the three messages from the rendered version of your Quarto document, whilst still ensuring that the "Package installation and loading" chunk is displayed in the rendered version and executed so that the libraries can be used in future code chunks?
...
[**Q8**]{style="color:green;"} Which code chunk option can you use to remove both the code of the "Package installation and loading" chunk and its outputs from the rendered version of your Quarto document, whilst still ensuring that the libraries are loaded so that their functions can be used further down in the document?
...
[^6]: If you look at the source code of this tutorial, you may notice that it sometimes makes use of double curly brackets `{{r}}` to introduce inline code. This syntax is only necessary in tutorials: it means that the rendered version of the document will include the code fencing `{r}` in the output (similar to the `echo: fenced` option for code chunks, see @sec-Chunks).
...
Inline code should only be used for very simple code, ideally with no more than one function, as in `` `{{r}} nrow(L1.data)` ``. To insert the output of more complex operations, it is best to write the code and temporarily save its output(s) in a hidden code chunk (using the option `#| include: false`, see @sec-Chunks). The following code chunk makes use of the R pipe operator (`|>`), which enables to us to seamlessly combine functions [see @lefollDataAnalysisLanguage2025: [Section 7.5.2](https://elenlefoll.github.io/RstatsTextbook/7_VariablesFunctions.html#sec-Piping)].
...
@fig-RealisticPipeline includes a caption, an alt-text, and a custom width as a percentage of page width. Note that, in the source code, special characters such as quotation marks need to be escaped using a backslash `\`. Tags beginning with `#fig-` can be used to cross-reference images by replacing the `#` with `@`. Hence, in this chapter, `@fig-RealisticPipeline` in the Quarto source code is rendered as @fig-RealisticPipeline.
...
#| fig-cap: "L2 participants' English lexical knowledge and professional occupational group"
#| fig-asp: 0.618
#| fig-width: 6
#| out-width: 90%
#| message: false
...
If you are sharing your research analyses and results in HTML format, you can also embed **interactive plots** [see @lefollDataAnalysisLanguage2025: [Section 10.2.8](https://elenlefoll.github.io/RstatsTextbook/10_Dataviz.html#sec-InteractivePlots)] in your Quarto documents. In [HTML version](https://elenlefoll.quarto.pub/quarto4research/#fig-scatterplot-plotly) of this tutorial, it is possible to hover over @fig-scatterplot-plotly to explore the data interactively.
...
![Screenshot of the [interactive plot](https://elenlefoll.quarto.pub/quarto4research/#fig-scatterplot-plotly) of L2 participants' lexical knowledge in English](images/IntPlotCh14a.png){#fig-scatterplot-plotly fig-alt="A screenshot of an interactive scatterplot with x-axis labeled as vocabulary test scores and y-axis labeled as collocation test scores. Each point represents an individual, colored by occupational group. Users can hover over each point to view additional details: VocabR, CollocR, L1, Age, Years in formal education, Job, OccupGroup."}
:::
...
The easiest way to "give credit where credit is due" is to use the [{grateful}](https://pakillo.github.io/grateful/) package. Its `cite_packages()` function will scan your project for all the R packages that are used and generate a BibTeX file called `grateful-refs.bib` that contains the package references. Having first installed the package, load the {grateful} library:
...
If you intend to share a longer Quarto document, it may be a good idea to number the headings and sub-headings (`number-sections`) and to include a table of contents (`toc`). You can do this by adding the following two lines to the `format` section of your YAML header:
...
: Presentation slide formats {#tbl-Slides tbl-colwidths="\[20,55,25\]"}
